...
func test_default_initialisation():
	...
	assert_eq(prereq_instance.chance,0.0,"chance should be 0")
	assert_eq(prereq_instance.events,[],"Events should be empty list")


func test_custom_initialisation():
	var custom_prereq_instance = Prerequisite.new(["TaskID"],0.5,[Event.EventType.AFTERSHOCK])
	assert_eq(custom_prereq_instance.task_id.size(), 1, "Task ID should contain 1 element")
	assert_eq(custom_prereq_instance.task_id[0], "TaskID", "First element should be TaskID")
