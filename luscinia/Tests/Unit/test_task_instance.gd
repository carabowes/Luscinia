...
func before_each():
	task_data = TaskData.new("TaskID","Test",null,Vector2(0,0),{"funds":10, "supplies": 50},{"funds":15},5,[])
...
func test_task_data_initialisation():
	...
	assert_eq(task_data.resources_required, {"funds":10, "supplies": 50}, "Resources required should match the dictionary")
...
func test_initialisation():
	...
	assert_eq(task_instance.actual_resources, {"funds":0, "supplies": 50}, "Actual resources should match the dictionary")
...
func test_get_completion_rate():
	assert_eq(task_instance.get_completion_rate(), 4.0/7.0, "Completion rate should be current progress (4) / total time (7)")


func test_update_task():
	task_instance.update_task()
	assert_eq(task_instance.current_progress, 5)
	assert_eq(task_instance.actual_resources["supplies"], 14)
	task_instance.update_task()
	task_instance.update_task()
	assert_true(task_instance.is_completed)
