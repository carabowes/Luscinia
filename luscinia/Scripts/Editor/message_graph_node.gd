...
func _init(message : Message):
	...
	# Adds message text input, with a callback for content change
	...
	# Adds default response input, with a callback for changes
	var default_response_input = add_input("Default Response Index", str(message.default_response))
	default_response_input.text_changed.connect(func(text):\
	_on_default_response_changed(text, default_response_input))

	# Sets up prerequisites and antirequisites ports
	add_spacer()
	...
	# Adds turns to answer input, with a callback for changes
	add_spacer()
	var turns_to_answer_input = add_input("Turns to Answer", str(message.turns_to_answer))
	turns_to_answer_input.text_changed.connect(func(text):\
	_on_turns_to_answer_changed(text, turns_to_answer_input))

	# Adds a checkbox for repeatability, with a callback for changes
	add_spacer()
	...
	# Adds a choice picker for cancel behavior, with a callback for changes
	add_spacer()
	var cancel_behaviour = add_choice_picker("Cancel Behaviour", Message.CancelBehaviour,\
	Message.CancelBehaviour.keys()[message.cancel_behaviour])
	cancel_behaviour.value_chosen.connect(func(value): message.cancel_behaviour = value;\
	print(message.cancel_behaviour))


# Callback for when the message contents are changed
func _on_message_contents_changed(contents : String):
	message.message = contents


# Callback for when the default response index is changed
...
# Callback for when the turns to answer value is changed
...
# Callback for when the repeatable checkbox value is changed
