...
func link_buttons():
	...
	var sender_creation = func(sender : Sender): 
	...
	%NewSender.pressed.connect(func(): add_node(Sender.new, SenderGraphNode.new, sender_nodes.append, sender_nodes.erase))
...
func load_sender(sender : Sender, message_node : MessageGraphNode):
	if sender == null:
		return
