...
enum CancelBehaviour {
	...
	PREREQ_RESEND, ##Put the message back into the send pool, will be sent when prerequisites become true
	ACT_AS_COMPLETED, ##When the task is cancelled, the task still enters the tasks completed pool
	PICK_DEFAULT ##Marks the default response option as completed rather than the task that was cancelled, if there is not a valid default_response picked this will behave the same as NOTHING
}
## Message contents
@export var message: String
## Player responses
@export var responses: Array[Response]
## Index of the default response, if -1 or out of range of the responses there is no default response
